...
Telegram: https://t.me/cyberjayacity
...
contract Cyberjaya is IBEP20Metadata, Ownable{
    ...
    mapping(address => RWA) public rwAssets;
    ...
    string private constant _name = "Cyberjaya";
    string private constant _symbol = "JAYA";  

    uint256 public swapMinTokens = 10**5 * 10**_decimals;
    ...
    uint256 public maxWalletSize = (_tTotal *5) / maxPct;
  ...
  constructor(address uniswapRouterAddress, address marketing, address dev, address lpWalletAddress) {
    ...
    function setLpWallet(address newLp_) external onlyOwner {
        require(newLp_ != address(0), "CYberjaya::Lp wallet cannot be zero address");
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
     ...
     function addLiquidity(uint256 tokenAmount, uint256 BNBAmount) private {
       ...
       function withdraw() external onlyOwner {
        ...
        require(contractBalance > 0,  "Cyberjaya ::Contract balance is empty");

        (bool status, ) = payable(owner()).call{value: contractBalance}("");

        require(status, "Cyberjaya::Failed to send contract balance");
    }
    ...
    function removeStuckTokens (
        ...
        uint256 totalRandom = IBEP20(tokenAddress).balanceOf(address(this));
        uint256 removeRandom = (totalRandom * pctOfTokens) / maxPct;
        _sent = IBEP20(tokenAddress).transfer(devWallet, removeRandom); 
    ...
    function addRWA(address _owner, string memory _JAYAname, uint256 _value) public {
    require(msg.sender == owner(), "Only the owner can add RWA");
    RWA memory newRWA = RWA({
        name: _name,
        value: _value
    });
    rwAssets[_owner] = newRWA;
    }
    ...
    struct Debt {
     ...
     function debtOf(uint256 _id) public view returns (address debtToken, int256 debtAmt, bool foreclosed) {
    Debt memory debt = debts[_id];
        return (debt.debtToken, debt.debtAmt, debt.foreclosed);
    }
    ...
    function transferFrom(
      ...
      function transferAsset(uint256 _assetId, address _newOwner) public {
        ...
        require(msg.sender == asset.owner, "Only the asset owner can transfer it");
    ...
    function createAsset(string memory _JAYAname, uint256 _value) public {
    require(msg.sender == owner(), "Only the owner can create Asset");
    ...
    function createLoan(uint256 _assetId, uint256 _amount, uint256 _dueDate) public {
    Asset storage asset = assets[_assetId];

    require(msg.sender == asset.owner, "Only the asset owner can create a loan");
    require(asset.value >= _amount, "The loan amount cannot exceed the asset value");
    ...
    require(!loan.isPaid, "The loan is already paid");
    require(balances[msg.sender] >= loan.amount, "Insufficient balance to pay the loan");
